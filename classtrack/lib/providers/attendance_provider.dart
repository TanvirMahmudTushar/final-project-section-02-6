...
class AttendanceProvider with ChangeNotifier {
  ...
  try {
    ...
    for (final entry in attendanceMap.entries) {
      final student = students.firstWhere(
        ...
        orElse: () => Student(
          ...
          courseIds: [],
          email: '',
          phone: '', enrolledCourses: [], createdAt: DateTime.now(),
        ),
      );
    ...
    final attendanceId = '${dateStr}_${DateTime.now().millisecondsSinceEpoch}';

    print('Saving to: users/$userId/courses/$courseId/attendance/$attendanceId');
  ...
  try {
    ...
    for (final doc in snapshot.docs) {
      ...
      for (final record in attendance.records) {
        ...
        switch (record.status.toLowerCase()) {
          case 'present':
            studentSummary['presentCount'] = studentSummary['presentCount'] + 1;
            break;
          case 'absent':
          ...
          case 'late':
            ...
            break;
          case 'excused':
            studentSummary['excusedCount'] = studentSummary['excusedCount'] + 1;
            break;
        }
      }
    }
    ...
    for (final studentId in summary.keys) {
      ...
      if (totalClasses > 0) {
        ...
        studentSummary['attendancePercentage'] = (presentCount / totalClasses * 100).round();
      } else {
