...
class _TakeAttendanceScreenState extends State<TakeAttendanceScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < studentProvider.students.length; i++) {
      ...
      print('  - Has courseIds field: ${student.courseIds != null}');
      print('  - courseIds type: ${student.courseIds.runtimeType}');
      print('  - courseIds content: ${student.courseIds}');
      print('  - courseIds length: ${student.courseIds.length}');

      // Check if courseId exists in courseIds
      final hasCourse = student.courseIds.contains(widget.courseId);
      print('  - Contains course ${widget.courseId}? $hasCourse');

      // If not found, print what's actually in courseIds
      if (!hasCourse && student.courseIds.isNotEmpty) {
        print('  - Actual course IDs in student:');
        for (var courseId in student.courseIds) {
          print('      * $courseId');
        }
      }
    }
    ...
    final enrolledStudents = studentProvider.students
        .where((student) {
          final isEnrolled = student.courseIds.contains(widget.courseId);
